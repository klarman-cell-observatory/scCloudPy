...
def select_hvg_seurat_single(X, n_top=None, min_disp=0.5, max_disp=np.inf, min_mean=0.0125, max_mean=7):
    ...
    if n_top is None:
        ...
        idx = np.logical_and.reduce(
            (mean > min_mean, mean < max_mean, log_disp_zscore > min_disp, log_disp_zscore < max_disp))
        hvg_rank[~idx] = -1
    else:
...
def select_hvg_seurat_multi(X, channels, cell2channel, n_top, n_jobs=1):
    ...
    res_arr = np.array(
        Parallel(n_jobs=n_jobs)(delayed(select_hvg_seurat_single)(Xs[i], n_top) for i in range(channels.size)))
    selected = res_arr >= 0
...
def select_hvg_seurat(data, consider_batch, n_top=2000, min_disp=0.5, max_disp=np.inf, min_mean=0.0125, max_mean=7,
                      n_jobs=1):
    robust_idx = data.var['robust'].values
    X = data.X[:, robust_idx]

    hvg_rank = select_hvg_seurat_multi(X, data.uns['Channels'], data.obs['Channel'], n_top,
        n_jobs=n_jobs) if consider_batch \
        else select_hvg_seurat_single(X, n_top=n_top, min_disp=min_disp, max_disp=max_disp, min_mean=min_mean,
        max_mean=max_mean)
...
def find_variable_features(data, consider_batch=False, flavor='scCloud', n_top=2000, span=0.02, plot_hvg_fig=None,
                           min_disp=0.5, max_disp=np.inf, min_mean=0.0125, max_mean=7, n_jobs=1):
    start = time.time()
    ...
    if flavor == 'scCloud':
       select_hvg_scCloud(data, consider_batch, n_top=n_top, span=span, plot_hvg_fig=plot_hvg_fig)
        ...
        select_hvg_seurat(data, consider_batch, n_top=n_top, min_disp=min_disp, max_disp=max_disp, min_mean=min_mean,
            max_mean=max_mean, n_jobs=n_jobs)
