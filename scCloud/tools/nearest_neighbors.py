...
def calculate_nearest_neighbors(X, num_threads, method='hnsw', K=100, M=20, efC=200, efS=200, random_state=0,
                                full_speed=False):
    ...
    if method == 'hnsw':
        ...
        knn = NearestNeighbors(n_neighbors=K - 1,
            n_jobs=num_threads)  # eliminate the first neighbor, which is the node itself
        knn.fit(X)
...
def neighbors(data, rep_key='X_pca', K=15, n_jobs=1, random_state=0):
    _ = get_kNN(data, rep_key, K, n_jobs, random_state, full_speed=True)
...
def get_kNN(data, rep_key, K, n_jobs=1, random_state=0, full_speed=False):
    ...
    if need_calc:
        indices, distances = calculate_nearest_neighbors(data.obsm[rep_key], n_jobs, K=K, random_state=random_state,
...
def calc_kBET(data, attr, rep_key='X_pca', K=25, alpha=0.05, n_jobs=1, random_state=0, temp_folder=None):
    ...
    kBET_arr = np.concatenate(Parallel(n_jobs=n_jobs, max_nbytes=1e7, temp_folder=temp_folder)(
        delayed(calc_kBET_for_one_chunk)(knn_indices[starts[i]:starts[i + 1], :], attr_values, ideal_dist, K) for i in
        range(n_jobs)))
