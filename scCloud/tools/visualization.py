...
def calc_tsne(X, n_jobs, n_components, perplexity, early_exaggeration, learning_rate, random_state, init='random',
              n_iter=1000, n_iter_early_exag=250):
    tsne = TSNE(n_jobs=n_jobs, n_components=n_components, perplexity=perplexity, early_exaggeration=early_exaggeration,
...
def calc_fitsne(X, nthreads, no_dims, perplexity, early_exag_coeff, learning_rate, rand_seed, initialization=None,
                max_iter=1000, stop_early_exag_iter=250, mom_switch_iter=250):
    ...
    return FItSNE(X.astype('float64'), nthreads=nthreads, no_dims=no_dims, perplexity=perplexity,
        early_exag_coeff=early_exag_coeff, learning_rate=learning_rate, \
        rand_seed=rand_seed, initialization=initialization, max_iter=max_iter,
        stop_early_exag_iter=stop_early_exag_iter, mom_switch_iter=mom_switch_iter)


# Running umap using our own kNN indices
def calc_umap(X, n_components, n_neighbors, min_dist, spread, random_state, init='spectral', n_epochs=None,
              learning_rate=1.0, knn_indices=None, knn_dists=None):
    umap_obj = um.UMAP(n_components=n_components, n_neighbors=n_neighbors, min_dist=min_dist, spread=spread,
    ...
    if X.shape[0] < 4096 or knn_indices is None:
        ...
        _init = check_array(umap_obj.init, dtype=np.float32, accept_sparse=False) if isinstance(umap_obj.init,
            np.ndarray) else umap_obj.init
...
def calc_force_directed_layout(W, file_name, n_jobs, target_change_per_node, target_steps, is3d, memory, random_state,
    ...
    classpath = pkg_resources.resource_filename('scCloud',
        'ext/forceatlas2.jar') + ':' + pkg_resources.resource_filename('scCloud', 'ext/gephi-toolkit-0.9.2-all.jar')
    command = ['java', '-Djava.awt.headless=true', '-Xmx{memory}g'.format(memory=memory), '-cp', classpath, \
               ...
               '--nthreads', str(n_jobs), '--seed', str(random_state), '--targetChangePerNode',
               str(target_change_per_node), '--targetSteps', str(target_steps)]
    if not is3d:
        command.append('--2d')
...
def tsne(data, rep_key='X_pca', n_jobs=None, n_components=2, perplexity=30, early_exaggeration=12, learning_rate=1000,
         random_state=0, out_basis='tsne'):
    if n_jobs is None:
        n_jobs = os.cpu_count()
    start = time.time()
    X_tsne = calc_tsne(data.obsm[rep_key], n_jobs, n_components, perplexity, early_exaggeration, learning_rate,
        random_state)
...
def fitsne(data, rep_key='X_pca', n_jobs=None, n_components=2, perplexity=30, early_exaggeration=12, learning_rate=1000,
           random_state=0, out_basis='fitsne'):
    if n_jobs is None:
        n_jobs = os.cpu_count()
    start = time.time()
    data.obsm['X_' + out_basis] = calc_fitsne(data.obsm[rep_key], n_jobs, n_components, perplexity, early_exaggeration,
        learning_rate, random_state)
    end = time.time()
    print("FItSNE is calculated. Time spent = {:.2f}s.".format(end - start))


def umap(data, rep_key='X_pca', n_components=2, n_neighbors=15, min_dist=0.5, spread=1.0, random_state=0,
         out_basis='umap'):
    start = time.time()
    ...
    data.obsm['X_' + out_basis] = calc_umap(data.obsm[rep_key], n_components, n_neighbors, min_dist, spread,
        random_state, knn_indices=knn_indices, knn_dists=knn_dists)
    end = time.time()
    print("UMAP is calculated. Time spent = {:.2f}s.".format(end - start))


def force_directed_layout(data, file_name, n_jobs, K=50, target_change_per_node=2.0, target_steps=5000, is3d=False,
                          memory=8, random_state=0, out_basis='fle'):
    start = time.time()
    realK = min(K - 1, data.uns['diffmap_knn_indices'].shape[1])  # K - 1: exclude self
    W = calculate_affinity_matrix(data.uns['diffmap_knn_indices'][:, 0:realK],
        data.uns['diffmap_knn_distances'][:, 0:realK])
    data.obsm['X_' + out_basis] = calc_force_directed_layout(W, file_name, n_jobs, target_change_per_node, target_steps,
        is3d, memory, random_state);
    end = time.time()
    print("Force-directed layout is calculated. Time spent = {:.2f}s.".format(end - start))


def net_tsne(data, rep_key, selected, n_jobs, n_components=2, perplexity=30, early_exaggeration=12,
             learning_rate=1000, random_state=0, net_alpha=0.1, polish_learning_frac=0.33, polish_n_iter=150,
             out_basis='net_tsne'):
    start = time.time()
    ...
    Y_init[~selected, :] = net_train_and_predict(X, X_tsne, data.obsm[rep_key][~selected, :], net_alpha, random_state,
        verbose=True)

    data.obsm['X_' + out_basis + '_pred'] = Y_init

    polish_learning_rate = polish_learning_frac * data.shape[0]
    data.obsm['X_' + out_basis] = calc_tsne(data.obsm[rep_key], n_jobs, n_components, perplexity, early_exaggeration,
        polish_learning_rate, random_state, \
        init=Y_init, n_iter=polish_n_iter, n_iter_early_exag=0)
    end = time.time()
    print("Net tSNE is calculated. Time spent = {:.2f}s.".format(end - start))


def net_fitsne(data, rep_key, selected, n_jobs, n_components=2, perplexity=30, early_exaggeration=12,
               learning_rate=1000, random_state=0, net_alpha=0.1, polish_learning_frac=0.5, polish_n_iter=150,
    ...
    Y_init[~selected, :] = net_train_and_predict(X, X_fitsne, data.obsm[rep_key][~selected, :], net_alpha, random_state,
        verbose=True)
...
def net_umap(data, rep_key, selected, n_jobs, n_components=2, n_neighbors=15, min_dist=0.1, spread=1.0,
             random_state=0, ds_full_speed=False, net_alpha=0.1, polish_n_epochs=30, polish_learning_rate=10.0,
             out_basis='net_umap'):
    start = time.time()
    X = data.obsm[rep_key][selected, :]

    if 'ds_knn_indices' not in data.uns:
        indices, distances = calculate_nearest_neighbors(X, n_jobs, K=n_neighbors, random_state=random_state,
    ...
    X_umap = calc_umap(X, n_components, n_neighbors, min_dist, spread, random_state, knn_indices=knn_indices,
        knn_dists=knn_dists)
    ...
    Y_init[~selected, :] = net_train_and_predict(X, X_umap, data.obsm[rep_key][~selected, :], net_alpha, random_state,
        verbose=True)
    ...
    data.obsm['X_' + out_basis] = calc_umap(data.obsm[rep_key], n_components, n_neighbors, min_dist, spread,
        random_state, \
        init=Y_init, n_epochs=polish_n_epochs, learning_rate=polish_learning_rate, knn_indices=knn_indices,
        knn_dists=knn_dists)
    end = time.time()
    print("Net UMAP is calculated. Time spent = {:.2f}s.".format(end - start))


def net_fle(data, selected, file_name, n_jobs, K=50, target_change_per_node=2.0, target_steps=10000, is3d=False,
            memory=8, random_state=0, \
    ...
    if 'diffmap_ds_knn_indices' in data.uns:
        ...
        indices, distances = calculate_nearest_neighbors(X, n_jobs, K=K, random_state=random_state,
            full_speed=ds_full_speed)
    ...
    X_fle = calc_force_directed_layout(W, file_name + '.small', n_jobs, target_change_per_node, target_steps, is3d,
        memory, random_state);
    ...
    Y_init[~selected, :] = net_train_and_predict(X, X_fle, data.obsm['X_diffmap'][~selected, :], net_alpha,
    ...
    W = calculate_affinity_matrix(data.uns['diffmap_knn_indices'][:, 0:realK],
        data.uns['diffmap_knn_distances'][:, 0:realK])
    data.obsm['X_' + out_basis] = calc_force_directed_layout(W, file_name, n_jobs, target_change_per_node,
        polish_target_steps, is3d, memory, random_state, init=Y_init);
    end = time.time()
