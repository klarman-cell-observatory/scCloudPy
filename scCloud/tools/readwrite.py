...
def read_dropseq_file(input_file, genome):
    ...
    data = anndata.AnnData(X=csr_matrix(df.values.transpose()), obs={"obs_names": df.columns.values},
        var={"var_names": df.index.values, "genes": df.index.values})
...
def read_antibody_csv(input_csv):
    ...
    data = anndata.AnnData(X=csr_matrix(np.stack(stacks, axis=1)), obs={"obs_names": barcodes},
        var={"var_names": antibody_names})
...
def __read_mtx(path):
    ...
    var = pd.read_csv(os.path.join(parent_directory, 'features.tsv.gz' if is_10x_v3 else 'genes.tsv'), sep='\t',
        ...
        names=['gene_ids', 'index', 'feature_type'] if is_10x_v3 else ['gene_ids', 'index'])
    ...
    obs = pd.read_csv(os.path.join(parent_directory, 'barcodes.tsv.gz' if is_10x_v3 else 'barcodes.tsv'), sep='\t',
        header=None, index_col=0, names=['index'])
