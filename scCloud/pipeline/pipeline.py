...
import scCloud.pipeline.preprocessing
...
def run_pipeline(input_file, output_name, **kwargs):
    ...
    if not kwargs['cite_seq']:
        adata = tools.read_input(input_file, genome=kwargs['genome'],
            mode=('a' if (is_raw or kwargs['subcluster']) else 'r+'), select_singlets=kwargs['select_singlets'])
    else:
    ...
    if is_raw:
        ...
        scCloud.pipeline.preprocessing.filter_data(adata, output_filt=kwargs['output_filt'],
            plot_filt=kwargs['plot_filt'], plot_filt_figsize=kwargs['plot_filt_figsize'], \
            mito_prefix=kwargs['mito_prefix'], min_genes=kwargs['min_genes'], max_genes=kwargs['max_genes'],
            min_umis=kwargs['min_umis'], max_umis=kwargs['max_umis'], \
            percent_mito=kwargs['percent_mito'], percent_cells=kwargs['percent_cells'],
    ...
    if is_raw:
        if kwargs['select_hvg']:
            tools.find_variable_features(adata, kwargs['batch_correction'], flavor=kwargs['hvg_flavor'],
                n_top=kwargs['hvg_ngenes'], plot_hvg_fig=kwargs['plot_hvg'], n_jobs=kwargs['n_jobs'])
    ...
    # diffusion map
    if is_raw:
        tools.diffmap(adata, 'X_pca', n_jobs=kwargs['n_jobs'], n_components=kwargs['nDC'],
            alpha=kwargs['diffmap_alpha'], K=kwargs['diffmap_K'], solver=kwargs['diffmap_solver'],
            random_state=kwargs['random_state'], full_speed=kwargs['diffmap_full_speed'])

        import time
        start_time = time.time()
        tools.get_kNN(adata, 'X_diffmap', kwargs['diffmap_K'], n_jobs=kwargs['n_jobs'],
            random_state=kwargs['random_state'], full_speed=kwargs['diffmap_full_speed'])
        end_time = time.time()
        print("KNN for diffusion components is finished. Time spent = {:.2f}s.".format(end_time - start_time))

        adata.obsm['X_diffmap_pca'] = tools.reduce_diffmap_to_3d(adata.obsm['X_diffmap'],
    ...
    if ('kBET' in kwargs) and kwargs['kBET']:
        stat_mean, pvalue_mean, accept_rate = tools.calc_kBET(adata, kwargs['kBET_batch'], K=kwargs['kBET_K'],
            alpha=kwargs['kBET_alpha'], n_jobs=kwargs['n_jobs'])
        print("kBET stat_mean = {:.2f}, pvalue_mean = {:.4f}, accept_rate = {:.2%}.".format(stat_mean, pvalue_mean,
            accept_rate))
        # if kwargs['kBJSD']:
    ...
    if kwargs['run_approx_louvain']:
        tools.approximate_louvain(adata, 'X_' + kwargs['approx_louvain_basis'],
            affinity=kwargs['approx_louvain_affinity'], resolution=kwargs['approx_louvain_resolution'],
            n_clusters=kwargs['approx_louvain_nclusters'], \
            n_init=kwargs['approx_louvain_ninit'], n_jobs=kwargs['n_jobs'], random_state=kwargs['random_state'],
            temp_folder=kwargs['temp_folder'], class_label='approx_louvain_labels')

    if kwargs['run_approx_leiden']:
        tools.approximate_leiden(adata, 'X_' + kwargs['approx_leiden_basis'], affinity=kwargs['approx_leiden_affinity'],
            resolution=kwargs['approx_leiden_resolution'], n_clusters=kwargs['approx_leiden_nclusters'], \
            n_init=kwargs['approx_leiden_ninit'], n_jobs=kwargs['n_jobs'], random_state=kwargs['random_state'],
            temp_folder=kwargs['temp_folder'], class_label='approx_leiden_labels')

    if kwargs['run_louvain']:
        tools.louvain(adata, affinity=kwargs['louvain_affinity'], resolution=kwargs['louvain_resolution'],
            random_state=kwargs['random_state'], class_label=kwargs['louvain_class_label'])

    if kwargs['run_leiden']:
        tools.leiden(adata, affinity=kwargs['leiden_affinity'], resolution=kwargs['leiden_resolution'],
            n_iter=kwargs['leiden_niter'], random_state=kwargs['random_state'],
    ...
    if kwargs['run_net_umap']:
        ...
        tools.run_net_umap(adata, 'X_pca', selected, n_jobs=kwargs['n_jobs'], n_neighbors=kwargs['umap_K'],
            ...
            polish_n_epochs=kwargs['net_umap_polish_nepochs'], out_basis=kwargs['net_umap_basis'])
    ...
    if kwargs['run_net_fle']:
        ...
        tools.run_net_fle(adata, selected, output_name, n_jobs=kwargs['n_jobs'], K=kwargs['fle_K'],
            ...
            net_alpha=kwargs['net_l2'], polish_target_steps=kwargs['net_fle_polish_target_steps'],
            out_basis=kwargs['net_fle_basis'])

    if kwargs['run_tsne']:
        tools.tsne(adata, 'X_pca', n_jobs=kwargs['n_jobs'], perplexity=kwargs['tsne_perplexity'],
            random_state=kwargs['random_state'])

    if kwargs['run_fitsne']:
        tools.fitsne(adata, 'X_pca', n_jobs=kwargs['n_jobs'], perplexity=kwargs['tsne_perplexity'],
            random_state=kwargs['random_state'])

    if kwargs['run_umap']:
        tools.umap(adata, 'X_pca', n_neighbors=kwargs['umap_K'], min_dist=kwargs['umap_min_dist'],
        ...
        new_data = anndata.AnnData(X=hstack([adata.X, csr_matrix(adt_matrix)], format='csr'),
            ...
            var={'var_names': var_names,
                 ...
                 'highly_variable_genes': np.concatenate(
                     [adata.var['highly_variable_genes'].values, [False] * cdata.shape[1]])
                 })
        new_data.obsm['CITE-Seq'] = adt_matrix
        adata = new_data
        print("ADT count matrix is attached.")

        tools.fitsne(adata, 'CITE-Seq', n_jobs=kwargs['n_jobs'], perplexity=kwargs['tsne_perplexity'],
            random_state=kwargs['random_state'], out_basis='citeseq_fitsne')
